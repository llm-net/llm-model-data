...
2. 比较模型、协议面、思考能力和价格正文；仅价格没变不能跳过协议核对。协议支持表、接入方式或型号范围变化也生成候选，忽略导航 / 广告噪声。
3. 聚合平台按现有catalog逐项核对，新型号先列建议；执行服务商与型号两层排除清单，硅基流动与 Together AI 不得自动恢复，不全量灌入。每天跨峰谷不改价，只在标准基准变化时更新。按AGENTS.md更新具体offering和证据，网络失败不清空模型、不刷新成功日期。
