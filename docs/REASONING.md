...
`coverage=partial` 不等于没有可执行选项。支持操作的消费者须逐 profile / control 判断；已核实的 API 参数应显示选择器，不能因默认或关闭能力未知而整模型禁用。Codex 的逐型号档位可核对官方发行程序的内嵌目录，证据注明版本与摘要；单独确认请求字段，不从 UI 标签推导。客户端默认与裸 API 默认分开，Ultra 的自动委派不是单次请求参数。
